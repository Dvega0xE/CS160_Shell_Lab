 ...
 * 
 * Diego Alejandro Vega - 861047657
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
void eval(char *cmdline)
{
    char *argv[MAXARGS];               //argv execve()
    int bg = parseline(cmdline, argv); //bg = 1 --> the job is to run in the bg
                                       //bg = 0 --> the job runs in the foreground
    sigset_t mask;
    pid_t pid;
    if (argv[0] == NULL)
        return; //input nothing, do nothing
    if (!(builtin_cmd(argv)))
    { //not a built-in cmd, need a child process from fork()
        //TODO
        sigemptyset(&mask);
        ...
        pid = fork();
        if (pid == 0)
        {                                          //only the child process enters this branch
            sigprocmask(SIG_UNBLOCK, &mask, NULL); //a child first unblock the signals
            setpgid(0, 0);                         //set a unique id for all new processes
            if (execve(argv[0], argv, environ) < 0)
            {
                printf("Not a valid command!\n");
                exit(0);
            } //it is possible to type an illegal command
        }
        //TODO
        //in the parent proc, we add the job into the job list, then we unblock the signals such that all the signals could run again
